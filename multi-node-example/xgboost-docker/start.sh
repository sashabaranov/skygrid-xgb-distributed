...
sudo docker run -t -i --rm=true --net=host --privileged -e "nfs_server=192.168.202.51" \
  -e "nfs_port=7000" -e "host=${local_addr}" kochetovnicolai/mpi-node /init.sh &
task=$(jobs | grep 'kochetovnicolai/mpi-node /init.sh')
echo $task
task=$(echo $task | cut -d ' ' -f 1 | cut -d '[' -f 2 | cut -d ']' -f 1)
echo $task
sudo docker run -t  --rm=true --net=host -e "host=192.168.202.51" \
    -e "client=${local_addr}" \
